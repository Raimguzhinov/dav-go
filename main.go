...
func main() {
	...
	calendarB := caldav.Calendar{Path: "/user/calendars/b", SupportedComponentSet: []string{"VTODO"}}
	calendars := []caldav.Calendar{
		caldav.Calendar{Path: "/user/calendars/a"},
		calendarB,
	}
	...
	handler := caldav.Handler{
		Backend: &backend{
			calendars: calendars,
			objectMap: map[string][]caldav.CalendarObject{
				calendarB.Path: []caldav.CalendarObject{object},
			},
		},
		Prefix: "",
	}
